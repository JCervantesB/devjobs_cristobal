        ...
        <form wire:submit.prevent='leerDatosFormulario'>
            <div class="md:grid md:grid-cols-3 gap-5">
                ...
                <div class="mb-5">
                    ...
                    <select value="" wire:model="categoria" class="border-gray-300 p-2 w-full">
                        <option>--Seleccione--</option>

                        @foreach ($vacantes as $vacante)
                            <option wire:key='categoria-{{ $vacante->id }}' value="{{ $vacante->categoria }}">
                                {{ $vacante->categoria }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-5">
                    <label value="" class="block mb-1 text-sm text-gray-700 uppercase font-bold">Salario
                        Anual</label>
                    <select wire:model="salario" class="border-gray-300 p-2 w-full">
                        <option>-- Seleccione --</option>
                        @foreach ($vacantes as $vacante)
                            <option wire:key='salario-{{$vacante->id}}' value="{{ $vacante->salario }}">{{ $vacante->salario }}</option>
                        @endforeach
                    </select>
                </div>
