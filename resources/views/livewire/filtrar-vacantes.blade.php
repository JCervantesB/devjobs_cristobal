                ...
                <div class="mb-5">
                    ...
                    <select  value="" wire:model="categoria" class="border-gray-300 p-2 w-full">
                        ...
                        @foreach ($vacantes as $vacante )
                            <option value="{{ $vacante->categoria }}">{{ $vacante->categoria }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-5">
                    <label value="" class="block mb-1 text-sm text-gray-700 uppercase font-bold">Salario Anual</label>
                    <select wire:model="salario" class="border-gray-300 p-2 w-full">
                        <option>-- Seleccione --</option>
                        @foreach ($vacantes as $vacante)
                            <option value="{{ $vacante->salario }}">{{$vacante->salario}}</option>
                        @endforeach
                    </select>
                </div>
